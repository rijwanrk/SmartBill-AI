...
const parseInvoiceFromText = async (req, res) => {
  ...
  try {
    ...
    - NEVER use dollar ($) symbol
    - Format numbers in Indian format (e.g., ₹1,00,000)
...
const generateReminderEmail = async (req, res) => {
  ...
  try {
    ...
    const prompt = `
      ...
      - Amount Due: ₹${invoice.total.toLocaleString("en-IN")}
...
const getDashboardSummary = async (req, res) => {
  ...
  try {
    ...
    const dataSummary = `
      ...
      - Total outstanding amount from unpaid/pending invoices: ₹${totalOutstanding.toLocaleString("en-IN")}
      - Recent invoices (last 5): ${invoices.slice(0, 5).map(inv => 
    `Invoice #${inv.invoiceNumber} for ₹${inv.total.toLocaleString("en-IN")} with status ${inv.status}`
  ).join(', ')}
    `;
